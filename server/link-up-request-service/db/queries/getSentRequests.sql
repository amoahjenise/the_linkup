SELECT
  ...
  (SELECT avatar FROM users WHERE id = link_up_requests.receiver_id) AS receiver_avatar
FROM
  link_up_requests
  INNER JOIN link_ups ON link_up_requests.linkup_id = link_ups.id
  INNER JOIN users ON link_up_requests.receiver_id = users.id
WHERE
  link_up_requests.requester_id = $1::uuid
  AND link_up_requests.hidden <> true
  ORDER BY
  link_up_requests.created_at DESC; -- Sort in descending order
